...
function startTimer(id) {
  ...
  if (id === 1) {
    ...
    timerInterval1 = setInterval(() => {
      ...
      document.getElementById("timer1").textContent = `00:${pad(secondsPassed)}`;
    }, 1000);
  } else {
    startTime2 = now;
    ...
    timerInterval2 = setInterval(() => {
      ...
      document.getElementById("timer2").textContent = `00:${pad(secondsPassed)}`;
    }, 1000);
  }
}
...
function stopTimer(id) {
  ...
  const hijri = UmmalquraCalendar.toHijri(now);
  const date = `${hijri.hijriYear}/${pad(hijri.hijriMonth + 1)}/${pad(hijri.hijriDay)}`;
